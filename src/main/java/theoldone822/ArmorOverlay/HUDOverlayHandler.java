...
//			if (ArmorOverlay.armorPices == 4)
	...
	public static void drawAromrOverlay(float al, Minecraft mc, int left, int top) {
        ...
        float toughness = (float) (mc.thePlayer.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue() > ArmorOverlay.tOverlayLevels ? ArmorOverlay.tOverlayLevels : mc.thePlayer.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS).getAttributeValue());
		...
		if (ArmorOverlay.tOverlayLevels > 0 && toughness > 0)
			mc.ingameGUI.drawTexturedModalRect(left + 0, top, 0, 81, (int) ((toughness / ArmorOverlay.tOverlayLevels) * 81), 9);
		if (ArmorOverlay.showNumbers){
				mc.ingameGUI.drawString(mc.fontRendererObj, Float.toString(al) + "A" + Float.toString(toughness) + "T", left + 82, top + 1, 0xffffff);
		}
